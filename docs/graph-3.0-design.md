...
当前实现边界：已具备 RawAsset/SourceElement/Observation、Claim/EvidenceLink、保守实体图、显式模式语义关系、多证据关系聚合、SQLite 权威元数据、多路 lexical/numeric/vector/graph 召回、EvidencePack 槽位覆盖与确定性追问。向量检索当前是可重建的 SQLite brute-force 基线，语义关系仍是低风险规则候选；ZenBrain、LLM 关系抽取和 ANN 索引尚未接入。
